...
func getCurrentProcessSecurityDescriptor() (*windows.SECURITY_DESCRIPTOR, error) {
	...
	for _, g := range gs.AllGroups() {
		...
		// log.Warn().Msgf("######################## %+v - %v - %v - %v - %v", g, !g.Sid.IsValid(), g.Sid.IdentifierAuthority() != windows.SECURITY_NT_AUTHORITY, g.Sid.SubAuthorityCount(), g.Sid.SubAuthority(0))
		if !g.Sid.IsValid() || g.Sid.IdentifierAuthority() != windows.SECURITY_NT_AUTHORITY {
			continue
		}
		sid = g.Sid
		break

...
func (di *Dependencies) bootstrapTequilapi(nodeOptions node.Options, listener net.Listener) (tequilapi.APIServer, error) {
	...
	tequilapi_endpoints.AddRoutesForIdentities(router, di.IdentityManager, di.IdentitySelector, di.IdentityRegistry, di.ConsumerBalanceTracker, di.AddressProvider, di.HermesChannelRepository, di.BCHelper, di.Transactor, di.BeneficiaryProvider, di.IdentityMover, di.PayoutAddressStorage)
	...
	tequilapi_endpoints.AddRoutesForProposals(router, di.ProposalRepository, di.PricingHelper, di.LocationResolver, di.FilterPresetStorage, di.NATProber)
	...
	tequilapi_endpoints.AddRoutesForNAT(router, di.StateKeeper, di.NATProber)
	...
	tequilapi_endpoints.AddRoutesForEthInfo(router, di.EtherClientL2)

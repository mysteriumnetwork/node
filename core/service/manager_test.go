...
func TestManager_StartRemovesServiceFromPoolIfServiceCrashes(t *testing.T) {
	...
	_, err := manager.Start(identity.FromAddress(proposalMock.ProviderID), serviceType, market.ACL{}, struct{}{})
	assert.Nil(t, err)
...
func TestManager_StartDoesNotCrashIfStoppedByUser(t *testing.T) {
	...
	id, err := manager.Start(identity.FromAddress(proposalMock.ProviderID), serviceType, market.ACL{}, struct{}{})
	assert.Nil(t, err)
	err = manager.Stop(id)
	assert.Nil(t, err)
	discovery.Wait()

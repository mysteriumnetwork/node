...
func (m *dialerHTTP) Dial(ctx context.Context, consumerID, providerID identity.Identity, serviceType string, contactDef ContactDefinition, tracer *trace.Tracer) (Channel, error) {
	...
	config, err := m.startConfigExchange(ctx, config, contactDef, providerID, serviceType, consumerID)
	if err != nil {
	...
	err = m.ackConfigExchange(ctx, config, contactDef, providerID, serviceType, consumerID)
	if err != nil {
...
func (m *dialerHTTP) startConfigExchange(ctx context.Context, config *p2pConnectConfig, contactDef ContactDefinition, providerID identity.Identity, serviceType string, consumerID identity.Identity) (*p2pConnectConfig, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("could not pack signed message: %w", err)
	}
	url := fmt.Sprintf("%s/%s/msg/%s?type=%s", contactDef.BrokerAddresses[0], serviceType, providerID.Address, MsgTypeInit)
...
func (m *dialerHTTP) ackConfigExchange(ctx context.Context, config *p2pConnectConfig, contactDef ContactDefinition, providerID identity.Identity, serviceType string, consumerID identity.Identity) error {
	...
	url := fmt.Sprintf("%s/%s/msg/%s?type=%s", contactDef.BrokerAddresses[0], serviceType, providerID.Address, MsgTypeAck)
	_, err = m.sendSignedMsg(ctx, url, packedMsg)

	if err != nil {
		return fmt.Errorf("could not send signed msg: %w", err)
	}
...
func (m *dialerHTTP) prepareLocalPorts(config *p2pConnectConfig) (string, []int, error) {
	...
	if err != nil {
		return publicIP, nil, fmt.Errorf("could not acquire local ports: %w", err)
	}
...
func (m *dialerHTTP) sendSignedMsg(ctx context.Context, url string, msg []byte) ([]byte, error) {
	...
	if err := json.NewDecoder(resp.Body).Decode(&events); err != nil {
		if err == io.EOF { // Skip empty response.
			return []byte{}, nil
		}
		return nil, err
	}
